...
export function generateFakeLockedCount(tableName: string): number {
  ...
  const query = useQuery<VillageContentResult<T>>({
    ...
    queryFn: async (): Promise<VillageContentResult<T>> => {
      ...
      try {
          ...
          if (villageId && table !== 'histoires') params.append('villageId', villageId);
      ...
      // Full pagination for both premium and free users
    ...
    refetchOnMount: 'always', // Always refetch on mount to ensure pagination is up to date
