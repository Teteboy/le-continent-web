...
import { ArrowLeft, MessageSquare, BookOpen, Clock, UtensilsCrossed, Type, MessageCircle, ExternalLink, Crown, Lock, Heart } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { useAuth } from '@/hooks/useAuth';
import { useQueryClient } from '@tanstack/react-query';
import { FREE_LIMIT, ITEMS_PER_PAGE, generateFakeLockedCount } from '@/hooks/useVillageContent';
...
function generateFakeLockedCountLocal(tableName: string): number {
...
export default function VillageOptionsPage() {
  ...
  useEffect(() => {
    ...
    tableConfig.forEach(({ table, orderBy }) => {
      ...
      resilientFetch(`${API_BASE}/api/content/${table}?${params.toString()}`)
        .then(res => res.json())
        .then(data => {
  ...
  const handleHover = async (tableId: string) => {
    ...
    if (village && queryClient) {
      ...
      try {
        ...
        const response = await resilientFetch(`${API_BASE}/api/content/${table}?${params.toString()}`);
        if (response.ok) {
