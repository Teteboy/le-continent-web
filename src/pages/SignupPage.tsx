...
export default function SignupPage() {
  ...
  const fetchProfileFromCache = async (userId: string) => {
    try {
      ...
      const response = await resilientFetch(`${API_BASE}/api/auth/profile/${userId}`, {
        method: 'GET',
  ...
  const handlePromoClose = () => {
    setShowPromo(false);
    // Check if we need to redirect to payment after signup
    if (redirectAfterSignup === 'payment') {
      setShowPayment(true);
      return;
    }
