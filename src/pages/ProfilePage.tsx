...
export default function ProfilePage() {
  ...
  // Check if we should show payment modal immediately (after signup redirect)
  useEffect(() => {
    if (searchParams.get('show_payment') === 'true' && !profile?.is_premium) {
      setShowPayment(true);
      // Clean up URL
      navigate('/profil', { replace: true });
    }
  }, [searchParams, profile?.is_premium, navigate]);
  ...
  const [withdrawMethod, setWithdrawMethod] = useState<'mtn' | 'orange' | null>(null);
  const [withdrawLoading, setWithdrawLoading] = useState(false);

  // Real referrals from Supabase
  const { data: referrals = [], isLoading: referralsLoading } = useReferrals(user?.id);

  const paginatedReferrals = referrals.slice((referralsPage - 1) * REFERRALS_PER_PAGE, referralsPage * REFERRALS_PER_PAGE);
  const totalReferralPages = Math.ceil(referrals.length / REFERRALS_PER_PAGE);

  const totalEarnings = referrals.reduce((acc, ref) => acc + (ref.referral_earnings ?? 0), 0);
  ...
  const handleWithdrawal = () => {
    const balance = profile?.balance || 0;
    if (balance < 500) {
      toast.error('Vous ne pouvez pas retirer moins de 500 FCFA');
      return;
    }
    // Open WhatsApp for withdrawal request
    const message = `Demande de retrait de ${balance} FCFA`;
    window.open(`https://wa.me/237672549955?text=${encodeURIComponent(message)}`, '_blank');
  };
          ...
          <>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-bold text-[#2C3E50]">Portefeuille</h3>
              </div>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <p className="text-sm text-gray-600">Solde disponible</p>
                  <p className="text-2xl font-bold text-[#8B0000]">{(profile?.balance || 0).toLocaleString()} FCFA</p>
                </div>
                <Button
                  onClick={handleWithdrawal}
                  disabled={(profile?.balance || 0) < 2000}
                  className="bg-[#8B0000] hover:bg-[#6B0000] text-white font-bold px-6 h-11 rounded-xl"
                >
                  Retirer
                </Button>
              </div>
              {(profile?.balance || 0) < 2000 && (
                <p className="text-xs text-gray-500">
                  Minimum 2 000 FCFA requis pour un retrait
                </p>
              )}
            </div>
              ...
              <div className="mt-4">
                <div className="bg-white/10 rounded-xl px-4 py-2 mb-3 flex items-center gap-2">
                  ...
                  <p className="text-white/90 text-xs font-medium">
                    Retrait possible à partir de <span className="font-black text-white">500 FCFA</span>
                  </p>
                </div>
                {(profile?.balance || 0) >= 2000 ? (
                  <Button
                  ...
                  >
                    <DollarSign size={18} /> Retirer mes gains ({(profile?.balance || 0).toLocaleString('fr-FR')} FCFA)
                  </Button>
                ) : (
                  <Button
                    disabled
                  ...
                  >
                    <DollarSign size={18} /> Encore {(2000 - (profile?.balance || 0)).toLocaleString('fr-FR')} FCFA avant le retrait
                  </Button>
                )}
              </div>
            </div>
          ...
          <div className="relative bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-gray-500 text-sm mt-1">
                Solde disponible : <span className="font-bold text-[#27AE60]">{(profile?.balance || 0).toLocaleString('fr-FR')} FCFA</span>
              </p>
            ...
            <div className="space-y-4">
              ...
              <Button
                onClick={async () => {
                  ...
                  if (balance < 2000) { toast.error('Minimum de retrait : 2 000 FCFA'); return; }
                  ...
                  try {
                    ...
                    if (res.data?.success || res.success) {
                      ...
                      toast.success('Demande de retrait soumise !', {
                        description: res.data?.message || `${balance} FCFA seront transférés sur ${withdrawPhone} sous 24-48h.`,
                      });
                      // Refresh profile to reflect updated balance
                      window.location.reload();
                    } else {
                      toast.error(res.data?.error || res.error || 'Échec de la demande de retrait');
                    }
                  } catch (err: unknown) {
                    const errorMessage = err instanceof Error ? err.message : 'Erreur réseau. Réessayez.';
                    toast.error(errorMessage);
                  } finally {
                    setWithdrawLoading(false);
                  }
                }}
                disabled={withdrawLoading || balance < 2000}
              ...
              >
                {withdrawLoading ? (
                  ...
                  <><Wallet size={18} /> Confirmer le retrait — {balance}FCFA</>
                )}
              </Button>
            </div>
          </div>
