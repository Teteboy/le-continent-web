...
export default function HistoirePage() {
  ...
  return (
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4">
          ...
          {!isLoading && total > 0 && (
            <p className="text-gray-500 text-sm">
              {isPremium ? `${startItem}-${endItem} sur ${total} histoires` : `${unlockedItems.length} histoires gratuites`}
          ...
          <div>
            ...
            {!isPremium && lockedOnPage > 0 && (
              <LockedItemsList<Histoire>
                items={Array(lockedOnPage).fill(null).map((_, i) => ({ id: `locked-${i}` } as Histoire))}
    ...
    renderItem={() => <button className="w-full flex items-center gap-4 p-4 text-left pointer-events-none">
      <div className="w-20 h-20 rounded-xl bg-[#9B59B6]/10 flex items-center justify-center shrink-0">
        <span className="text-[#9B59B6] text-2xl">📖</span>
      </div>
      <div className="flex-1 min-w-0">
